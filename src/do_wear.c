...
Cloak_on()
{
	...
	case CLOAK_OF_INVISIBILITY:
		...
		if (!oldprop && !See_invisible && !Blind) {
			makeknown(uarmc->otyp);
			newsym(u.ux,u.uy);
			pline("Suddenly you cannot %s.", see_yourself);
		}
		break;
	case OILSKIN_CLOAK:
...
Cloak_off()
{
	...
	case MUMMY_WRAPPING:
		if (Invisible && !Blind) {
			newsym(u.ux,u.uy);
			You("can no longer %s.", see_yourself);
		}
		break;
	case CLOAK_OF_INVISIBILITY:
		if (!oldprop && !See_invisible && !Blind) {
			...
			newsym(u.ux,u.uy);
			pline("Suddenly you can %s.", see_yourself);
		}
		break;
